...
def _positronium_source_default_direction():
  return Box(
      {
        "type": "iso",
...
def _positronium_source_default_energy():
  return Box(
      {
        "type": "mono",
        "mono": 0,
...
def _setter_hook_positronium_source_particle(self, particle):
...
class PositroniumSource(SourceBase, g4.GatePositroniumSource):
    ...
    user_info_defaults = {
        "particle": (
          "gamma",
          {
            ...
            "setter_hook": _setter_hook_positronium_source_particle,
            },
          ),
        "ion": (
          Box({"Z": 0, "A": 0, "E": 0}),
        ...
        "direction": (
            _positronium_source_default_direction(),
    ...
    def __init__(self, *args, **kwargs):
        print("in __init__ of PositroniumSource")
    ...
    def initialize(self, run_timing_intervals):
        if not isinstance(self.position, Box):
            fatal(
                f"positronium Source: user_info.position must be a Box, but is: {self.position}"
            )
        if not isinstance(self.direction, Box):
            fatal(
                f"positronium Source: user_info.direction must be a Box, but is: {self.direction}"
            )
        if not isinstance(self.energy, Box):
            fatal(
                f"positronium Source: user_info.energy must be a Box, but is: {self.energy}"
            )
        ...
        # initialize
        # print("before SourceBase.initialize(self, run_timing_intervals)")
        # print(run_timing_intervals)
        # print(f"self.user_info={self.user_info}")
        # print("************")
        # self.user_info={'particle': 'gamma', 'ion': {'Z': 0, 'A': 0, 'E': 0}, 'weight': -1, 'weight_sigma': -1, 'user_particle_life_time': -1, 'tac_times': None, 'tac_activities': None, 'direction_relative_to_attached_volume': False, 'position': {'type': 'sphere', 'radius': 5.0, 'sigma_x': 0, 'sigma_y': 0, 'size': [0, 0, 0], 'translation': [-30.0, 300.0, -30.0], 'rotation': ([[1., 0., 0.],
       # [0., 1., 0.],
       # [0., 0., 1.]]), 'confine': None}, 'direction': {'type': 'momentum', 'theta': [0, 3.141592653589793], 'phi': [0, 6.283185307179586], 'momentum': [0, -1, 0], 'focus_point': [0, 0, 0], 'sigma': [0, 0], 'acceptance_angle': {'skip_policy': 'SkipEvents', 'volumes': [], 'intersection_flag': False, 'normal_flag': False, 'normal_vector': [0, 0, 1], 'normal_tolerance': 0.05235987755982989}, 'accolinearity_flag': False, 'accolinearity_fwhm': 0.008726646259971648, 'histogram_theta_weights': [], 'histogram_theta_angles': [], 'histogram_phi_weights': [], 'histogram_phi_angles': []}, 'energy': {'type': 'mono', 'mono': 1.0, 'sigma_gauss': 0, 'is_cdf': False, 'min_energy': None, 'max_energy': None, 'spectrum_type': None, 'spectrum_weights': [], 'spectrum_energies': [], 'spectrum_energy_bin_edges': [], 'spectrum_histogram_interpolation': None}, 'attached_to': 'world', 'mother': None, 'start_time': None, 'end_time': None, 'n': 0, 'activity': 1e-05, 'half_life': -1, 'name': 'source1'}
        # print(f"self.user_info={self.user_info}")
        SourceBase.initialize(self, run_timing_intervals)
        print("after SourceBase.initialize(self, run_timing_intervals)")
